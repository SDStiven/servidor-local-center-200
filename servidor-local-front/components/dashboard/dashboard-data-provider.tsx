...
async function getRatesByUserId(id: string, token: string): Promise<RatesApiData | null> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_DATA_API_URL}/prestador/get-preco-hora/${id}`, {
    method: "GET",
    headers: {
...
async function getProposal(idUser: string, token: string): Promise<ProposalType[]> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_DATA_API_URL}/proposal/get-by-user-id/${idUser}`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
